...
function AnimatedCode() {
  ...
  <div className="bg-[#1e1e1e] rounded-lg border border-gray-800 overflow-hidden shadow-2xl shadow-blue-500/10">
    ...
    {/* Code Content */}
    <div className="py-5 px-0 font-mono text-xs md:text-sm leading-relaxed h-[360px] md:h-[340px] md:p-6 overflow-hidden">
      {displayedLines.map((line, index) => {
        const safeLineNumber = line && line.trim() ? index + 1 : "";
        return (
          <div key={index} className="flex items-center min-h-[1.5rem]">
            <span className="text-gray-500 w-8 text-right mr-2 md:mr-4 select-none">{safeLineNumber}</span>
            <span
              className="text-gray-400"
              dangerouslySetInnerHTML={{ __html: renderCodeLine(line || "") }}
            />
